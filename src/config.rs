...
pub struct Config {
    ...
    pub slippage: [f64; 2],
    pub gtd_expiration_secs: u64, // GTD订单过期时间（秒），默认900秒（15分钟）；仅当 arbitrage_order_type=GTD 时有效
    ...
    /// 窗口结束前收尾：距离当前15分钟窗口结束还有多少分钟时触发收尾（取消挂单→Merge→市价卖剩余）。0=不启用。
