...
async fn main() -> Result<()> {
    // rustls 0.23+ 需在首次 TLS 使用前安装默认 CryptoProvider
    ...
    tracing::info!("Polymarket 15分钟套利机器人启动");
    ...
    loop {
        ...
        use chrono::Utc;
        use crate::market::discoverer::FIFTEEN_MIN_SECS;
        let current_window_timestamp = MarketDiscoverer::calculate_current_window_timestamp(Utc::now());
        let window_end = chrono::DateTime::from_timestamp(current_window_timestamp + FIFTEEN_MIN_SECS, 0)
        ...
        loop {
            ...
            // 使用秒级精度，15分钟窗口下 num_minutes() 截断可能导致漏检
            ...
            tokio::select! {
                                ...
                                if let Some(total_price) = total_ask_price {
                                    if total_price <= execution_threshold {
                                        if let Some(opp) = _detector.check_arbitrage(
                                            ...
                                            // 使用秒级精度，15分钟市场下 num_minutes() 截断可能导致漏检
                ...
                // 定期检查：1) 是否进入新的15分钟窗口 2) 收尾触发（15分钟窗口需更频繁检查）
