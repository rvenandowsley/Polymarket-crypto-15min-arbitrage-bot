...
/// 15分钟窗口的秒数（供 main 等模块计算 window_end 使用）
pub const FIFTEEN_MIN_SECS: i64 = 900;
...
impl MarketDiscoverer {
    ...
    /// 计算当前15分钟窗口的开始时间戳（UTC）
    /// 窗口对齐到每15分钟整点：0, 15, 30, 45 分
    pub fn calculate_current_window_timestamp(now: DateTime<Utc>) -> i64 {
        let ts = now.timestamp();
        (ts / FIFTEEN_MIN_SECS) * FIFTEEN_MIN_SECS
    }

    /// 计算下一个15分钟窗口的开始时间戳（UTC）
    pub fn calculate_next_window_timestamp(now: DateTime<Utc>) -> i64 {
        let ts = now.timestamp();
        ((ts / FIFTEEN_MIN_SECS) + 1) * FIFTEEN_MIN_SECS
    }

    /// 生成市场slug列表
    /// 15分钟市场格式：btc-updown-15m-1771818300
    pub fn generate_market_slugs(&self, timestamp: i64) -> Vec<String> {
        self.crypto_symbols
            .iter()
            .map(|symbol| format!("{}-updown-15m-{}", symbol, timestamp))
            .collect()
    }
